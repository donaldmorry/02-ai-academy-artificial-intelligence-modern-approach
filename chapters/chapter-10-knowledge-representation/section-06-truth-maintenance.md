# Section 10.6: Truth Maintenance

> **Source:** Russell & Norvig, *Artificial Intelligence: A Modern Approach* (4th ed.), Knowledge Representation
...
**Truth Maintenance** is part of AIMA's broader program of building rational agents: represent the task clearly, compute with the representation, and evaluate whether the resulting behavior improves expected performance.

For this chapter, the working frame is **structured knowledge representation**. The practical objective is to **model objects, events, categories, defaults, and exceptions without losing inference discipline**. That phrasing matters: AIMA is not just a catalog of algorithms; it is a discipline for matching problem structure to a representation and then choosing an inference, learning, or decision procedure.
...
1. Define **Truth Maintenance** using agent-centered language.
...
| Data | entities, relations, events, rules, defaults, and provenance annotations | Makes assumptions explicit |
...
| Evaluation | competency questions, consistency checks, exception tests, and explanation traces | Turns correctness into observable evidence |
...
$$
> **Readable form:** A universal rule says every cat is a mammal.

$$
Default(x) \land \neg Exception(x) \Rightarrow Conclusion(x)
$$
> **Readable form:** A default conclusion applies only when no known exception blocks it.

$$
Belief_{t+1}=\operatorname{Revise}(Belief_t, evidence)
$$
> **Readable form:** Truth-maintenance systems update beliefs when new evidence arrives.
...
```python
def apply_default(facts, rule):
    premise, exception, conclusion = rule
    if premise in facts and exception not in facts:
...
1. Build the smallest possible instance of **Truth Maintenance**.
2. Solve it by hand.
...
1. What is the representation used by **Truth Maintenance**?
...
**Previous:** [Section 10.5: Default Reasoning](./section-05-default-reasoning.md)  
**Next:** [Section 10.7: The Frame Problem](./section-07-the-frame-problem.md)
